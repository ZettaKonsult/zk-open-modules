  ...
  describe("Parse file.", () => {
    ...
    it("First and last line.", async () => {
      ...
      expect(cells[cells.length - 1]).toEqual([
        ...
        "Sidney",
        "Birchenough",
        "sbirchenoughrq@merriam-webster.com",
        "Male",
        "30.50.101.37"
      ])
    })
    describe("Options.", () => {
      describe("Skip.", () => {
        it("Skip first and last lines.", async () => {
          ...
          expect(cells[0]).toEqual([
            ...
            "Devon",
            "Wickins",
            "dwickinsb@aboutads.info",
            "Female",
            "81.81.161.31"
          ])
          expect(cells[length - 1]).toEqual([
            "987",
            "Rorke",
            "Teers",
            "rteersre@jigsy.com",
      ...
      describe("Limit", () => {
        it("Row limit only.", async () => {
          const cells = await parseFile("mocks/sample.csv", { lineLimit: 10 })
          const length = cells.length
          ...
          expect(cells[length - 1]).toEqual([
            ...
            "Geoffrey",
            "Ashborne",
            "gashborne8@utexas.edu",
            "Male",
            "25.32.250.62"
          ])
        })
        it("Zero limit.", async () => {
          let cells = await parseFile("mocks/sample.csv", { lineLimit: 0 })
      ...
      describe("Header", () => {
        it("Empty header.", async () => {
          const cells = await parseFile("mocks/sample.csv", { header: [] })
          const length = cells.length
        ...
        it("Many header fields.", async () => {
          const cells = await parseFile("mocks/sample.csv", {
            header: ["Wahoo", "Yahooo", "Wahaaa", "It's-ah-me", "Mama mia"]
          })
          const length = cells.length
      ...
      describe("Delimiter", () => {
        it("Only colon", async () => {
          ...
          expect(cells[0]).toEqual(["A;B", ";C;D", ";E"])
          expect(cells[1]).toEqual(["A", "B;", "C", "D;", "E"])
        })
